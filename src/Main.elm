...
getReposFromUrl : Cmd Msg
getReposFromUrl = Http.get {
    url = "https://api.github.com/users/Raul2245/repos",
...
returnDefaultRepo : Repo
returnDefaultRepo =  
    { name = "Error", 
    description = (Just "Error trying to fetch repositories"), 
    url = "https://api.github.com/users/Raul2245/repos", 
    pushedAt = "today", 
    stars = 0 }
...
update msg model =
    case msg of
        GetRepos ->
            ( model, getReposFromUrl )

        GotRepos res ->
            ( { model | repos = Result.withDefault [returnDefaultRepo] res} , Cmd.none )
