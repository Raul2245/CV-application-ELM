...
monthsBetween dA dB =
    let 
        ma = monthToInt (Maybe.withDefault Jan (month dA))
        ya = year dA
        mb = monthToInt (Maybe.withDefault Jan (month dB))
        yb = year dB
    in
        case month dA of
            Nothing -> case month dB of
                            Nothing -> Just (abs ((ya - yb) * 12))
                            _ -> Nothing
            _ -> case month dB of
                    Nothing -> Nothing
                    _ -> case compare dA dB of
                            LT -> Just ((mb - ma) + (yb - ya) * 12)
                            GT -> Just ((ma - mb) + (ya - yb) * 12)
                            EQ -> Just 0
...
compare (Date d1) (Date d2) = 
    if year (Date d1) < year (Date d2) then
        ...
        case month (Date d1) of
            Nothing -> case month (Date d2) of
                            Nothing -> EQ
                            Just _ -> GT
            Just _ -> case month (Date d2) of 
                        Nothing -> LT
                        Just _ -> ( if monthToInt (Maybe.withDefault Jan (month (Date d1))) > monthToInt (Maybe.withDefault Jan (month (Date d2))) then
                                        GT
                                  else if monthToInt (Maybe.withDefault Jan (month (Date d1))) < monthToInt (Maybe.withDefault Jan (month (Date d2))) then 
                                        LT
                                  else
                                    EQ
                                    )
...
view (Date d) = 
    div [] [
        p [] [text (case month (Date d) of
                        Nothing -> " Present "
                        Just _ -> monthToString (Maybe.withDefault Jan (month (Date d))))
            ]
        , p [] [text (String.fromInt (year (Date d)))]
        ]
...
dateToString (Date d) = 
    case (month (Date d)) of 
        Nothing -> " - " ++ String.fromInt (year (Date d))
        Just _ -> monthToString (Maybe.withDefault Jan (month (Date d))) ++ " " ++ String.fromInt (year (Date d))
